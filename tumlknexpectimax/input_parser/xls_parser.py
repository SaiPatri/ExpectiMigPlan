...
class Parser:
    ...
    def xls_parse_residential(self, data_file):
        ...
        opex_values = excel_file1.parse('OPEX').set_index('Technology')
        ...
        return parsed_adsl_0,parsed_fttc_1,parsed_fttb_2, parsed_fttb_3, parsed_ftth_4, parsed_ftth_5, parsed_fttc_6_force, parsed_fttb_7_force, parsed_fttb_8_force, parsed_fttc_hybridpon_9, parsed_fttb_hybridpon_10, parsed_ftth_hybridpon_11, parsed_fttc_hybridpon_force_12, parsed_fttb_hybridpon_force_13,capex_values, opex_values, mig_matrix
    ...
    def xls_parse_business(self, data_file):
        ...
        capex_values = excel_file1.parse('CAPEX').set_index('Technology Name') # CAPEX
        mig_matrix = excel_file1.parse('MIG_MATRIX').set_index('Technology') # MIG_MATRIX
        return capex_values, opex_values,mig_matrix


if __name__ == "__main__":
    parsed = Parser()
    capex, opex,mig_capex = parsed.xls_parse_business('input_data_business.xlsx')
    max_year = 2038
    current_year = 2018
    pen_curve = 'Aggr PV'
    # print sum([fttc1.loc[year][pen_curve] for year in range(current_year,max_year+1)])
    # TODO: USE THIS FOR OPEX
    print(opex['Approx OPEX per year'].to_dict())
